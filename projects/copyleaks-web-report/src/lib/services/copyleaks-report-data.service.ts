...
export class CopyleaksReportDataService {
  private _reportEndpointConfig$ = new BehaviorSubject<
    IClsReportEndpointConfigModel | undefined
  >(undefined);
  /**
  ...
  private _scanResults$ = new BehaviorSubject<IScanResult[] | undefined>(
    undefined
  );
  /**
  ...
  private _crawledVersion$ = new BehaviorSubject<IScanSource | undefined>(
    undefined
  );
  /**
   * Subject for sharing the report complete results.
  ...
  public initReportData(endpointsConfig: IClsReportEndpointConfigModel) {
    ...
    const crawledVersionReq = this._http.get<IScanSource>(
      endpointsConfig.crawledVersion
    );
    const completeResultsReq = this._http.get<ICompleteResults>(
      endpointsConfig.completeResults
    );
  ...
  public loadAllReportScanResults(completeResults: ICompleteResults) {
    ...
    if (completeResults?.results) {
      ...
      const sub = fetchResultsBatches
        .pipe(
          concatMap((ids) =>
            forkJoin(...ids.map((id) => this.getReportResultAsync(id)))
          )
        )
        .subscribe((results) => {
          if (results) {
            ...
            const fetchedResults: IScanResult[] =
              this._scanResults$.value ?? [];
            this._scanResults$.next([
              ...fetchedResults,
              ...results,
            ] as IScanResult[]);
          }
  ...
  public async getReportResultAsync(resultId: string) {
    ...
    var requestUrl = this._reportEndpointConfig$.value.result.replace(
      '{RESULT_ID}',
      resultId
    );
