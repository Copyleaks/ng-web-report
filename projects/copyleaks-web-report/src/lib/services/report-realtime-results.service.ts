...
export class ReportRealtimeResultsService {
	...
	public pushNewResults(newResults: ResultPreview[]): void {
		this._reportDataService.reportEndpointConfig$.pipe(filter(value => value !== undefined)).subscribe(_ => {
						...
						if (currentBatchIndex === totalBatches) {
							const mappedResults = this._loadedResultsDetails$.map(resultDetail => {
								...
								return {
									...
									metadataSource: {
										words: this._reportDataService.scanResultsPreviews?.scannedDocument.totalWords ?? 0,
										excluded: this._reportDataService.scanResultsPreviews?.scannedDocument.totalExcluded ?? 0,
									},
								} as IResultItem;
							});
